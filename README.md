...
### zustand 추가 이유
일단 기본적으로 서버상태관리 툴과 클라이언트 사이드의 글로벌 전역 상태관리를 따로 하는 편 입니다. 리액트 쿼리의 캐싱 및 제공해주는 기능들이 강력하기 때문인데요. 전역 상태관리로 zustand를 선택한 이유는 기본적으로는 recoil, zotai 또는 Context API 등을 비교해봤을 때에 가장 업데이트가 활발하면서도 사용하는 유저수가 2번째로 많기 때문입니다. 또한 리코일의 업데이트나 빠른 대응이 늦어질 수도 있다는 점 또한 결국 zustand나 recoil도 보일러 플레이트나 적용 코드가 간단하다는 점은 거의 비슷한데, recoil의 경우에는 전역변수를 변경하는 함수를 컴포넌트 안에서 바로 작성할 수 있어서 편하지만, 코드베이스가 커지고, 비즈니스로직이 추가되다보면 이에 대한 액션 및 스토어 함수가 무거워 지게 되는 것을 고려했을 때에 분리하여 관리하는 것에 이점이 있다고 생각합니다.
